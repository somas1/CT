...
A logical value is commonly referred to as a boolean value. It is the result of a comparison between two variables. It is represented as `TR``UE` or `FALSE.`
...
1.  Create two strings with your first and last name, respectively. Then concatenate them, and print the new value as "My name is \<your newly created concatenated variable will go here\>"
    ...
    #paste("My name is",full)
    sprintf("My name is %s", full)
    ```

2.  What values can a BOOLEAN contain?

A BOOLEAN value can contain the values TRUE or FALSE.
